...
Singleton {
    ...
    function getWorkspace(slotOrId) {
        ...
        if (slot === undefined || slot < 0 || slot >= maxWorkspaces)
            return null;
    ...
    function getWindow(slotOrId) {
        ...
        if (slot === undefined || slot < 0 || slot >= maxWindows)
            return null;
    ...
    function getFocusedWorkspace() {
        if (focusedWorkspaceSlot < 0 || focusedWorkspaceSlot >= maxWorkspaces)
            return null;
    ...
    function getFocusedWindow() {
        if (focusedWindowSlot < 0 || focusedWindowSlot >= maxWindows)
            return null;
        const win = windowBuffer[focusedWindowSlot];
        return win.valid && win.isFocused ? win : null;
    }
