 ...
 * - Automatically invalidates cache when WindowStore.collectionVersion changes
 *
 * Usage:
...
Singleton {
    ...
    function getGroupsForWorkspace(workspaceId, forceRebuild) {
        ...
        if (cache && cache.version === WindowStore.collectionVersion && !forceRebuild) {
    ...
    function getWindowsInGroup(groupSlot) {
        if (groupSlot < 0 || groupSlot >= maxGroups)
            return [];
    ...
    function getPrimaryWindow(groupSlot) {
        if (groupSlot < 0 || groupSlot >= maxGroups)
            return null;
        const group = groupPool[groupSlot];
        if (!group.valid || group.primaryWindowSlot < 0)
            return null;
    ...
    function rebuildGroupsForWorkspace(workspaceId) {
        ...
        workspaceGroupCache[workspaceId] = {
            version: WindowStore.collectionVersion,
            groups: usedSlots
        };
